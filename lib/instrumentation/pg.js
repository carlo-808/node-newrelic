...
const NR_PG_CONN = Symbol('newrelic.pg.conn')
...
module.exports = function initialize(agent, pgsql, moduleName, shim) {
  ...
  shim.record(pgsql && pgsql.Client && pgsql.Client.prototype, 'query', function recordClient() {
    return {
      callback: shim.LAST,
      name: 'pg.Client.query',
      wrapper: function wrapCreator(shim, originalQueryMethod) {
        return function wrappedQuery() {
          this.connection[NR_PG_CONN] = {
            host: this.host,
            port_path_or_id: this.port,
            database_name: this.database
          }

          const originalReturn = originalQueryMethod.apply(this, arguments)
  ...
  // shim.wrap(pgsql && pgsql.Client && pgsql.Client.prototype,
  //   'query', function wrapCreator(shim, originalQueryMethod) {
  //     return function wrappedQuery() {
  //       const seg = shim.getActiveSegment()
  //       console.log('parent: ',seg.id)
  ...
  shim.recordQuery(
    pgsql && pgsql.Connection && pgsql.Connection.prototype,
    'query',
    function wrapJSClientQuery(shim) {
      return {
        ...
        query: getQuery,
        stream: 'row',
        parameters: getInstanceParameters(this),
        internal: false
      }
    }
  )
...
function getInstanceParameters(connection) {
  const instanceParams = connection[NR_PG_CONN]
  delete connection[NR_PG_CONN]
